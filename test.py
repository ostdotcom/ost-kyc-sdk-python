...
class TestStringMethods(unittest.TestCase):
    ...
    def test_create_new_user(self):
        ...
        test_obj.update(email='testuser+' +
                        str(int(time.time())) +
                        '_' +
                        full_python_version() +
                        '@ost.com')
        result = self.users_service.create(
            test_obj
        )
        DEBUG = os.environ.get('OST_KYC_SDK_DEBUG')
        if DEBUG:
            print (result)
        self.assertEqual(result['success'], True, "Create new user failed")

    def test_get_user(self):
        result = self.users_service.get({'id': Config.USER_ID})
        self.assertEqual(result['success'], True, "Get user is failed")

    def test_get_user_with_id_zero(self):
        result = self.users_service.get({'id': 0})
        self.assertEqual(result['success'], False, "Get user with id 0 failed")

    def test_get_user_with_id_blank(self):
        try:
            result = self.users_service.get({'id': '  '})
            self.fail("test get user with blank is failed")
        except Exception as error:
            self.assertEqual(
                str(error),
                'missing id',
                "test get user with blank is failed")

    def test_get_user_with_id_invalid(self):
        try:
            result = self.users_service.get({'id': 'BB&#@@#^@&#^@#^'})
            self.fail("test get user with blank is failed")
        except Exception as error:
            self.assertEqual(
                str(error),
    ...
    def test_get_user_list(self):
        result = self.users_service.list()
        self.assertEqual(result['success'], True, "Get users list is failed")

    def test_submit_kyc(self):
        result = self.users_kyc_service.submit_kyc(
            {
                'user_id': 11035,
                'first_name': 'aniket',
                'last_name': 'ayachit',
                ...
                'investor_proof_files_path': [
                    '2/i/9ff6374909897ca507ba3077ee8587da',
                    '2/i/4872730399670c6d554ab3821d63ebce']})
        self.assertEqual(result['success'], False, "Submit KYC is failed")

    def test_get_users_kyc_list(self):
        result = self.users_kyc_service.list()
        self.assertEqual(result['success'], True, "get users KYC list is failed")

    def test_get_users_kyc(self):
        result = self.users_kyc_service.get({'user_id': Config.USER_ID})
        self.assertEqual(result['success'], True, "get users KYC is failed")
    ...
    def test_validate_eth_address(self):
        ...
        self.assertEqual(result['success'], True, "Ethereum address is correct")

    def test_signature(self):
        http_helper_obj = HTTPHelper({})
        generated_signature = http_helper_obj.get_signature_for_test(
            Config.test_obj_for_signature,
            Config.test_endpoint,
            Config.API_SECRET_TO_TEST_SIGNATURE)
        self.assertEqual(
            Config.GENERATED_SIGNATURE,
            generated_signature,
            'Generated signature and given signature should be matched')
